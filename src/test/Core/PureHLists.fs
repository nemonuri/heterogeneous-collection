...
let toIntFolder (p: QuickHLists.IPredicate) = 
    { new IInRefFolder<int> with 
        member _.Step (acc, elem): int = 
...
let Test1() =
    ...
    let folder = toIntFolder { new QuickHLists.IPredicate with member _.Invoke (x: 'T): bool = QuickHLists.isLesserThan 0 x } in
